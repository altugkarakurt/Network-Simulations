...
##----------------------------------------------------------------------------------------
##      TIMER COALESCING ENERGY EFFICIENT ETHERNET SIMULATION	
##----------------------------------------------------------------------------------------	
...
class Frame(Process):
    ...
    def arrive(self, b):                              # frame arrives to link
        def inLine():                                 # checks if the waiting frames
													  #	can be serviced
            return first_in_queue == None             # checks if waiting frames other  
                                                      # than the 1st, can be serviced 
        ...
        global first_in_queue                         # which frame is first in queue
        arriveTime = now()                            # time stamp for frame arrival
        print("%8.12f %s: I arrived" % (now(), self.name)) 
        queue_length += 1                             # incremented due to the arrival
        if ((queue_length == 1) and (not(isActive))): # behavior of 1st arriving frame 
													  # in LPI state
            first_in_queue = self
            yield hold, self, T-Tw                    # waits for T-Tw
            ...
            first_in_queue = None                     # next in queue is enabled
